...
function app_units_query(BuildingId, userRole, isEdit) {
        ...
        activate: function (e, tab) {
            ...
            switch (tab.id) {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    if ($("#listAgent").is(':empty')) {
                        //loadFormSerachType("formAgent");
                        loadTabAgents();
                    }
                    break;
                case 3:
    ...
    var loadTabAgents = function () {
        ...
        var agentAdapter = new $.jqx.dataAdapter(agentSource, {
            contentType: "application/json; charset=utf-8",
            loadError: function (jqXHR, status, error) {
                //alert("dealAdapter failed: " + error);
            },
            ...
            formatData: function (data) {
                data.name_startsWith = $("#searchAgent").val();
                return data;
            }
        ...
        $('#listAgent').on('select', function (event) {
            var args = event.args;
            if (args) {
                var index = args.index;
                var item = args.item;
                $('#selectedAgentId').val(item.value);
            }
        });
    }
    ...
    $('.formGeneral').change(function () {
        var action = app_buildings.getFormAction("formGeneral");
        $("#formGeneral").attr('action', action);
    });
    $('.formAddress').change(function () {
        var action = app_buildings.getFormAction("formAddress");
        $("#formAddress").attr('action', action);
    });
    
    $('.formAgent').change(function () {
        var action = app_buildings.getFormAction("formAgent");
        $("#formAgent").attr('action', action);
        //}
    });

    $('.formOwner').change(function () {
        var action = app_buildings.getFormAction("formOwner");
        $("#formOwner").attr('action', action);
        //}
    });
    $('.formNatam').change(function () {
        var action = app_buildings.getFormAction("formNatam");
        $("#formNatam").attr('action', action);
        //}
    });

    function loadFormSerachType(form) {
        var action = app_buildings.getFormAction(form);
    ...
    $('#submitAddress').on('click', function () {
        ...
        app_jqx.inputAutoLabel("#building_name", "#building_name_val",true);

        //$("#building_name_val").val("");
        //var bname = "";
        //var item = $("#building_name").val();
        //if (item) {
        //    var val = item.label;
        //    bname = (val == null) ? "" : val.jsonEscape();
        //    $("#building_name_val").val(bname);
        //}
