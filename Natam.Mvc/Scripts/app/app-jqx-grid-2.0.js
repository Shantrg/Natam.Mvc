...
var app_jqxgrid = {
    loadEntityDataGrid: function (entity, data, tag_grid, pageable, popupTag) {
        ...
        $(tag_grid).on('rowdoubleclick', function (event) {
            ...
            app_jqxgrid.doRowEdit(boundIndex, entity, tag_grid, popupTag);
        });
    ...
    loadEntityGrid: function (entity, tag_grid, pageable, popupTag, loadCompleted) {
    ...
    renderToolbar: function (toolbar, entity, tag_grid, popupTag) {
        if (tag_grid === undefined || tag_grid == null)
            tag_grid = "#jqxgrid";
        ...
        $("#updaterowbutton").on('click', function () {
            ...
            app_jqxgrid.doRowEdit(selectedrowindex, entity, tag_grid, popupTag);
        ...
        $("#addrowbutton").on('click', function () {
            ...
            app_jqxgrid.openPopupEditor('popupWindow', tag_grid);
        });
        // delete row.
    ...
    doRowEdit: function (selectedrowindex, entity, tag_grid, popupTag) {
        if (tag_grid === undefined || tag_grid == null)
            tag_grid = "#jqxgrid";
        var editrow = -1;
        var rowscount = $(tag_grid).jqxGrid('getdatainformation').rowscount;
        if (selectedrowindex >= 0 && selectedrowindex < rowscount) {
            ...
            if (dataRecord != null && dataRecord !== undefined) {
                ...
                app_jqxgrid.openPopupEditor(popupTag, tag_grid);
            }
            // show the popup window.
    ...
    //openPopupEditor: function (mode, popupTag, tag_grid) {
    ...
    //    var tag = (popupTag === undefined || popupTag == null) ? "popupRegister" : popupTag;
            ...
            input.keypress(function (ev) {
                ...
                if (keycode == '13') {
                    if (input.val().length >= 2) {
                        ...
                        me.timer = setTimeout(function () {
                            app_jqxgrid.searchFilter($("#fieldsOption").val(), input.val(), 'contains');
                        }, 1000);
                    }
                    else {
                        app_jqxgrid.clearFilter();
        ...
        */
        ...
        if (dateList) {
            ...
            if (dateList.length == 1) {
                datelabel = dateList[0].label;
                datecolumn = dateList[0].value;
                datepicker.attr("title", "נא לבחור מתאריך עד תאריך [" + datelabel + "] ולהמתין לביצוע הסינון")
            }
            else
            {
                containerDate.append(datecollist);
    ...
    gridColumnsInit: function (columnList, tag_grid) {
        ...
        $(columnList).each(function (index, item) {
            $(tag_grid).jqxGrid('beginupdate');
            if (item.checked) {
                $(tag_grid).jqxGrid('showcolumn', item.value || item.label);
            }
            else {
                $(tag_grid).jqxGrid('hidecolumn', item.value || item.label);
            }
            $("#jqxgrid").jqxGrid('endupdate');
        });
